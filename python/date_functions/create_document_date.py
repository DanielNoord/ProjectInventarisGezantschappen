...
def create_document_date(
    ...
    if day and not month:
        raise ValueError(f"Document has day but no month, see: {row}")
    if day and not year:
        raise ValueError(f"Document has day but no year, see: {row}")
    if month and not year:
        raise ValueError(f"Document has month but no year, see: {row}")
    if any(isinstance(i, date) for i in (year, month, day)):
        raise TypeError(
            f"Some argument of document date has incorrect type (non-integer), see: {row}"
        )
    if year:
