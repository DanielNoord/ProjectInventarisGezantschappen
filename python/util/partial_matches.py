...
def find_partial_matches(print_to_file: bool, sanitize: bool) -> None:
    ...
    sanitized_dir = (
        str(input_dir).replace("inputs", "outputs").replace("VolumesExcel", "VolumesExcelSanitized")
    )
    ...
    for file in sorted(
        files,
        key=lambda name: int(name.replace("Paesi Bassi VOLUME ", "").replace("_it_IT.xlsx", "")),
    ):
