...
from functions.create_xml_elements import (
    basic_xml_file,
    dossier_entry,
...
from functions.parse_docx import split_into_dossiers as read_split_into_dossiers
from functions.parse_docx import sub_dossier_description as read_sub_dossier_description
...
def write_xml_file(localization, filename):
    ...
    for volume in extract_volumes(filename):
        ...
        for dossier in read_split_into_dossiers(volume):
            ...
            d_number, d_pages, d_title, d_date, d_serie_desc, files = read_dossier(
                dossier
            )
            c02 = dossier_entry(
                c01, v_number, d_number, d_pages, d_title, d_date, localization
            )
            ...
            if d_serie_desc:
                for sub_serie in read_sub_series(d_serie_desc):
                    ...
                    c03 = dossier_with_desc(
                        c02, d_d_pages, d_d_title, d_d_place, d_d_date, localization
                    )
                    if sub_serie[1]:
                        for file in sub_serie[2].split("\n"):
                            f_pages, f_title, f_place, f_date = read_file(file)
                            dossier_specific_file(
                                c03, f_pages, f_title, f_place, f_date, localization
                            )
